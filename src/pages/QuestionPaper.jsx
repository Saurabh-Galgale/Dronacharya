...
/**
 * QuestionPaper
 * - Keeps UI identical to your previous version
 * - Normalizes backend shapes for 'attempt' and 'review' responses
 * - Review mode: disabled inputs, selected options highlighted
 * - Attempt mode: interactive radios, options mapped correctly
 */
export default function QuestionPaper() {
  ...
  // answers map holds option TEXT for radios (so RadioGroup value is string)
  const [answers, setAnswers] = useState({});
  // submitted === true shows review-like UI (non-interactive)
  // For "review" backend we set submitted = true and populate answers from submission
  ...
  // Tag breakdown (unchanged behaviour)
  ...
  useEffect(() => {
    ...
    async function loadPaper() {
      setLoading(true);
      try {
        // read view param from url query: ?view=review | attempt | auto
        const searchParams = new URLSearchParams(window.location.search);
        const viewParam = (searchParams.get("view") || "auto")
          .toString()
          .toLowerCase();

        // call backend via api.getPaperView
        const res = await api.getPaperView(paperId, viewParam);

        if (!mounted) return;

        // Helper: normalize an "attempt" paper object where backend may put questions
        // under 'questions' or 'questionIds'.
        const normalizeAttemptPaper = (apiPaper) => {
          // Accept both forms:
          // - apiPaper.questions: [ { id/_id, questionText, options: [{text}], tag, explanation } ]
          // - apiPaper.questionIds: same array
          const sourceQuestions =
            Array.isArray(apiPaper.questions) && apiPaper.questions.length
              ? apiPaper.questions
              : Array.isArray(apiPaper.questionIds) &&
                apiPaper.questionIds.length
              ? apiPaper.questionIds
              : [];

          const mappedQuestions = sourceQuestions.map((q, idx) => {
            // options might be objects with { text } or strings
            ...
            return {
              // use a short sequential id for display (1-based) but keep rawId for reference
              id: q._id || q.id || String(idx + 1),
              rawId: q._id || q.id || String(idx + 1),
              question: q.questionText || q.question || q.questionText || "",
              options: opts,
              correctAnswer: null, // attempt payload usually shouldn't reveal correct answer
          ...
          return {
            id: apiPaper._id || apiPaper.id || String(paperId),
            name: apiPaper.name || apiPaper.title || "",
            subject: apiPaper.subject || "",
            durationMinutes: apiPaper.durationMinutes || apiPaper.duration || 0,
            totalMarks: apiPaper.totalMarks || apiPaper.totalMarks || 0,
            questions: mappedQuestions,
          };
        };

        // Helper: normalize a "review" submission response (submission.answers array)
        const normalizeReview = (submission) => {
          // submission.answers[] contains objects like your sample
          const mappedQuestions = Array.isArray(submission.answers)
            ? submission.answers.map((a, idx) => {
                ...
                // selectedOptions can be indices; map first index to text (radio)
                let selectedText = "";
                if (
                  Array.isArray(a.selectedOptions) &&
                  a.selectedOptions.length
                ) {
                  const selIdx = Number(a.selectedOptions[0]);
                  if (
                    !Number.isNaN(selIdx) &&
                    selIdx >= 0 &&
                    selIdx < opts.length
                  ) {
                    selectedText = opts[selIdx];
                  }
                } else {
                  // some backends might mark isSelected flags in options; find that
                  const sel = (a.options || []).find((o) => o && o.isSelected);
                  if (sel) selectedText = String(sel.text || "");
                }

                // normalize correctAnswer: could be index or text; convert to text if index
                let correctText = null;
                if (typeof a.correctAnswer === "number") {
                  const idxC = Number(a.correctAnswer);
                  if (!Number.isNaN(idxC) && idxC >= 0 && idxC < opts.length)
                    correctText = opts[idxC];
                } else if (typeof a.correctAnswer === "string") {
                  correctText = a.correctAnswer;
                }

                return {
                  id: a.questionId || a._id || String(idx + 1),
                  rawId: a.questionId || a._id || String(idx + 1),
                  question: a.questionText || a.question || "",
                  options: opts,
                  selectedOptions: Array.isArray(a.selectedOptions)
                    ? a.selectedOptions.map(Number)
                    : [],
                  selectedText,
          ...
          // Build answers map keyed by q.id -> option text (for UI)
          const answersMap = {};
          mappedQuestions.forEach((q) => {
            answersMap[q.id] = q.selectedText ?? "";
          });
          ...
          return {
            ...
            answersMap,
            meta: {
              paperId: submission.paperId || submission.paper || null,
              submittedAt:
                submission.submittedAt || submission.createdAt || null,
              totalMarksObtained: submission.totalMarksObtained ?? null,
              totalMarksPossible: submission.totalMarksPossible ?? null,
              percent: submission.percent ?? null,
            },
          };
        };

        // Decide based on res.mode or presence of submission/paper
        if (res.mode === "attempt" || (res.mode === "auto" && res.paper)) {
          const normalized = normalizeAttemptPaper(res.paper || res);
          setPaper(normalized);
          setSubmitted(false);
          setAnswers({}); // fresh attempt
          setResult(null);
        } else if (
          res.mode === "review" ||
          (res.mode === "auto" && res.submission)
        ) {
          const submission = res.submission;
          const paperMeta = res.paperMeta || res.paper || {};
          const normalized = normalizeReview(submission);

          setPaper({
            id: paperMeta.id || paperMeta._id || res.paperId || paperId,
            name:
              paperMeta.name || paperMeta.title || paperMeta.paperTitle || "",
            subject: paperMeta.subject || paperMeta.paperSubject || "",
            durationMinutes:
              paperMeta.durationMinutes ||
              paperMeta.duration ||
              submission.durationSeconds
                ? Math.ceil((submission.durationSeconds || 0) / 60)
                : 0,
            totalMarks:
              paperMeta.totalMarks || submission.totalMarksPossible || 0,
            questions: normalized.questions,
          });

          // answers map sets selected option text so radios can show that value
          setAnswers(normalized.answersMap);
          setSubmitted(true); // review should be non-interactive
          setResult({
          ...
          // unexpected shape but contains paper
          const normalized = normalizeAttemptPaper(res.paper);
          setPaper(normalized);
          setSubmitted(false);
          setAnswers({});
          setResult(null);
        } else {
          // nothing usable returned -> do not fallback to mock (per your instruction), clear paper
          setPaper(null);
          setSubmitted(false);
          setAnswers({});
          setResult(null);
        ...
        // don't fallback to mock automatically (you asked that). show console error and clear paper.
        console.error("Failed to load paper view:", err);
        setPaper(null);
        setSubmitted(false);
        setAnswers({});
        setResult(null);
  ...
  // reset state when paper id changes to a different internal id
  useEffect(() => {
    setAnswers({});
    setSubmitted(false);
    setResult(null);
  ...
  const handleChange = (qId, value) => {
    // Only allow changes when NOT submitted (interactive attempt)
    if (submitted) return;
      ...
      >
        ...
        <Box sx={{ overflowY: "auto", p: 2, flex: 1 }}>
          {(pageQuestions || []).map((q, idx) => (
            <Card key={q.id || q.rawId || idx} sx={{ mb: 2 }}>
              <CardContent>
                  ...
                  {(q.options || []).map((opt, i) => {
                    ...
                    return (
                      <FormControlLabel
                        ...
                        control={<Radio />}
                        label={optValue}
                        // disable if submitted (review) OR if paper came from review (submitted===true)
                        disabled={submitted}
                      />
                    );
                  })}
                ...
                {/* Show review details when submitted (review mode) */}
                {submitted && (
                  <Box sx={{ mt: 2 }}>
                    <Typography
                      variant="body1"
          ...
          {/* Load next page button for interactive attempt */}
              ...
              return (
                <Grid item key={q.id || q.rawId || i}>
                  <Paper
                    ...
                    onClick={() => {
                      // move to the page where this question lives and open sheet
                      const qIndex = i;
                      const newPage = Math.floor(qIndex / PAGE_SIZE);
                      setPage(newPage);
                      setSheetOpen(true);
                      setGridOpen(false);
                    }}
