...
const usePaymentGateway = () => {
  ...
  const handleBuyPackage = async (packageId, packageName) => {
    setLoading(true);
    const toastId = toast.loading("Initiating package purchase...");
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          try {
            ...
            const result = await verifyPackagePayment(verificationData);

            if (result.success) {
              // Update user profile in local storage with new purchased package
              const purchasedPackages = userProfile.purchasedPackages || [];
              if (!purchasedPackages.includes(packageId)) {
                purchasedPackages.push(packageId);
              }
              ...
              Object.keys(sessionStorage)
                .filter((key) => key.startsWith("packages_"))
  ...
  return { handleBuy, handleBuyPackage, loading };
};
